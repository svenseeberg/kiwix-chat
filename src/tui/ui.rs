...
fn render_message(lines: &mut Vec<Line<'static>>, msg: &DisplayMessage, width: usize, markdown: bool) {
    ...
    for raw in msg.text.split('\n') {
        ...
        let (marker, marker_style, segs, hang) = if markdown && in_code {
            (String::new(), style, vec![(raw.to_string(), code_block_style())], 0)
        } else if markdown {
            let b = classify_block(raw, style);
            (b.marker, b.marker_style, parse_inline(&b.text, b.style), b.hang)
        } else {
